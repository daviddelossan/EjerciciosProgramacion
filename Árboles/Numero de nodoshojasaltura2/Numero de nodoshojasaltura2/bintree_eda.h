...
public:
	...
	int calculaNodos(int numNodos) {

		if (!left().empty()) numNodos = left().calculaNodos(numNodos); //Si el izquierdo no está vacío, calculamos los nodos de ese hijo
		if (!right().empty()) numNodos = right().calculaNodos(numNodos); //Si el derecho no está vacío, calculamos los nodos de ese hijo

		if (root_->elem_ == '*') numNodos++;

		return numNodos;
	}

	int calculaHojas(int numHojas) {
		if (left().empty() && right().empty()) numHojas++;
		else {
			if (!left().empty()) numHojas = left().calculaHojas(numHojas);
			if (!right().empty()) numHojas = right().calculaHojas(numHojas);
		}

		return numHojas;
	}

	int calculaAltura(int altura) {

		if(!left().empty() && !right().empty()) altura = max(left().calculaAltura(altura), right().calculaAltura(altura));
		else if (!right().empty()) altura = right().calculaAltura(altura);
		else if (!left().empty()) altura = left().calculaAltura(altura);
