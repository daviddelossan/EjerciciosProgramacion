...
public:
	...
	void calculaFrontera(std::vector<int> &frontera) {
		int elementoFrontera = 0;

		if (left().empty() && right().empty()) {
			elementoFrontera = root_->elem_;
			frontera.push_back(elementoFrontera);
		}
		else {
			if (!left().empty()) left().calculaFrontera(frontera);
			if (!right().empty()) right().calculaFrontera(frontera);
		}
...
void resuelveCaso() {
	...
	if(!arbol.empty()) arbol.calculaFrontera(frontera);

	for (int i = 0; i < frontera.size(); ++i) {
		std::cout << frontera.at(i) << " ";
	}
