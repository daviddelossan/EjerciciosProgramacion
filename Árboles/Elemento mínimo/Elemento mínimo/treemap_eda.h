...
public:
	...
	bintree<T> leerArbol(T vacio) {
		...
		if (raiz == vacio) return{};
			...
			auto der = leerArbol(vacio);
			return{ iz, raiz, der };
		}
	}

	void devuelveMenor(T &menor) {
		if (root_->elem_ < menor) menor = root_->elem_;
		if(!left().empty()) left().devuelveMenor(menor);
		if (!right().empty()) right().devuelveMenor(menor);
	}
